# 展開担当者向け資料

このディレクトリは、AI Development Harnessをチームに展開する人向けの資料です。

メンバー向けに配る基本資料は `docs/`、`policies/`、`checklists/` にあります。この `facilitator/` は、説明会の準備、導入計画、進行、展開前チェックに使います。

## ファイル一覧

- [展開ロードマップ](./rollout-plan.md)
- [30分説明会プラン](./session-plan-30min.md)
- [30分説明会 台本たたき台](./explanation-script-30min.md)
- [展開前チェックリスト](./adoption-checklist.md)

## 初回説明会で扱う資料

30分しか取れない場合、全部は説明しません。

扱う範囲:

- `docs/harness-concept.md` の要点
- `docs/start-guide.md` のプロジェクトルートとVS Code注意点
- `policies/common.md` の禁止事項
- `docs/tool-selection-guide.md` の使い分け
- `docs/first-exercise.md` の最初の課題

説明会の目的は、全資料を理解してもらうことではありません。まず安全に起動し、変更なしで調査できる状態にすることです。

