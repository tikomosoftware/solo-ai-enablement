...
const documentTitle = "AI Project Template";
  ...
  return fs
    ...
    .flatMap((entry) => {
      ...
      if (entry.isDirectory()) {
        if (entry.name === "html" || entry.name === "scripts") {
          return [];
        }
        return listMarkdownFiles(absolute, baseDir);
      }
...
function build() {
  ...
  for (const page of pages) {
    ...
    const relativeIndexPath = path.relative(path.dirname(outputPath), path.join(htmlDir, "index.html")).replace(/\\/g, "/");
    const nav = `<nav><a href="${relativeIndexPath}">記事一覧</a> / ${escapeHtml(page.title)}</nav>`;
  ...
  const indexBody = [
    `<h1>${documentTitle}</h1>`,
    "<p>Markdown原本から生成した共有用HTMLです。</p>",
    "<ul>",
    ...pages.map(
      (page) => `<li><a href="./${page.output.replace(/\\/g, "/")}">${escapeHtml(page.title)}</a></li>`
    ),
    "</ul>",
  ].join("\n");
  ...
  console.log(`Generated ${pages.length + 1} HTML files in ai-project-template/html/`);
}
