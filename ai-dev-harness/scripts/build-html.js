  ...
  return fs
    ...
    .flatMap((entry) => {
      ...
      if (entry.isDirectory()) {
        if (entry.name === "html" || entry.name === "scripts" || entry.name === "assets") {
          return [];
        }
...
function build() {
  ...
  for (const page of pages) {
    ...
    const isIndex = page.source === "index.md";
    ...
    const nav = isIndex 
      ? `<nav>インデックス</nav>`
      : `<nav><a href="${relativeIndexPath}">インデックス</a> / ${escapeHtml(page.title)}</nav>`;
  ...
  console.log(`Generated ${pages.length} HTML files in ai-dev-harness/html/`);
}
