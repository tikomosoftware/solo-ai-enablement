# 16. Claude Codeを仕事で使うための安全設定サンプル

## この記事の目的

この記事では、開発者がClaude Codeを仕事で使うときに、最低限入れておきたいルールと設定の考え方を整理します。
...
`bypassPermissions` は権限確認をほぼ飛ばすモードです。公式ドキュメントでも、コンテナやVMのような隔離環境でのみ使うべきものとして説明されています。通常の実務PCや本物の実務リポジトリでは使わない方針にします。
...
AIにpushまで任せると便利ですが、仕事では「push前に人間が差分を見る」ほうが安全です。そのため、最初は `ask` にします。
...
Claude Codeを仕事で使うなら、`CLAUDE.md` と `.claude/settings.json` を分けて考えることが大切です。
