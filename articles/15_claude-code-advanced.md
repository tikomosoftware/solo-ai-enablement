...
公式ドキュメントでは、`settings.json` がClaude Codeの設定機構として説明されています。ユーザー全体に効く設定、プロジェクトで共有する設定、個人用のローカル設定、組織管理ポリシーの設定があります。
